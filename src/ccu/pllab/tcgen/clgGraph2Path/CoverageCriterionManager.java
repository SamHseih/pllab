...
public class CoverageCriterionManager implements CLGCoverageCriterion {
	...
	private String TD_path_dependency = "";
	private TestData_Path_Dependency TPD;
...
//							String tempConStr = "";
							...
							boundary = true;
							if(Main.TestType == "BlackBox") { //blackbox
								if(pathNocheck == pathNo)
									pathNo++;
								...
								if(pathNocheck == pathNo)
									pathNo++;
								DataWriter.testCons_output_path=DataWriter.output_folder_path+"\\test constraints\\"+graphClassName+"_WhiteBox_DCC\\";
								DataWriter.writeInfo(clpContent, graphClassName + graphMethodName + TestTypeSign + pathNo,
										"ecl", DataWriter.output_folder_path+"\\test constraints\\"+graphClassName+"_WhiteBox_DCC\\");
								pathNocheck = pathNo;
							}
							if (false) {
...
//		寫出路徑資料
		...
		if(Main.TestType == "WhiteBox") {
			DataWriter.testPath_output_path = DataWriter.output_folder_path+"\\test paths\\"+graphClassName+"_WhiteBox_"+Main.criterion+"\\";
		}
...
//		目前DUP部分未使用
		...
		if(Main.TestType == "WhiteBox") {
			DataWriter.testData_output_path = DataWriter.output_folder_path+"\\test data\\"+graphClassName+"_WhiteBox_"+Main.criterion+"\\";
		}
		DataWriter.writeInfo(content, graphClassName + graphMethodName, "txt",
				DataWriter.testData_output_path);
		if(!BlackBoxLauncher.generate_testinfo) { //版本控制專用 放test data那server裡
			InputStream targetStream = new ByteArrayInputStream(content.getBytes());
			FileHandleToServer.storeFile(BlackBoxLauncher.version_saveFilePath,graphClassName + graphMethodName+".txt", targetStream);
		}
		return this.testDatas;
	}
	
	//for verison control ~3547
	@SuppressWarnings("rawtypes")
	public List<TestData> genTestSuite_VersionControl(String version, String compareVer) throws IOException, CloneNotSupportedException {
		String path = "";
		String content = "";
		String hasSamePath;
		this.clpTranslator = new CLPTranslator();
...
//							String tempConStr = "";
							...
							if (false) {
								...
								clpSolver.solving(graphClassName, graphMethodName, pathNo, 1, graphIsConstructor,
										graphMethodRetType, "Obj_pre", "Arg_pre", "Obj_post", "Arg_post", "RetVal");
								testDatas.add(clpSolver.getTestData());
								TD_path_dependency += tempConStr2+"\n";
								TD_path_dependency += clpSolver.getCLPTestDataString()+"\n";
							} else {
								/* white-box: clp solves once. */
//								只有bound, unbound會用到，應該
								if (clpContent.contains("...") || clpContent.contains("Bound")
...
//		目前DUP部分未使用
		...
		System.out.println("ignore count is: "+ignore_cnt);
		return this.testDatas;
	}
...
//			}
	...
	public String getTD_path_dependency() {
		return this.TD_path_dependency;
	}
}
